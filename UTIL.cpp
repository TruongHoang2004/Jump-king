...
bool loadMedia( Tile* tiles[] )
{
    ...
    if ( !gTilesTexture.loadFromFile( "Jump king character/Tileset/Tileset.png") )
    {
...
bool setTiles( Tile * tiles[] , std::string path)
{
    ...
    else
    {
        ...
        if ( tilesLoaded )
        {
            ...
            {
                gTileClips[ i ] = {((i - 1) % TILE_OF_ROW) * TILE_WIDTH, ((i - 1) / TILE_OF_ROW ) * TILE_HEIGHT, TILE_HEIGHT, TILE_HEIGHT};
            }
        }
    }
...
bool touchesWall( SDL_Rect box, Tile* tiles[] )
{
    ...
    for( int i = 0; i < TOTAL_TILES; ++i )
    {
        ...
        if( ( tiles[ i ]->getType() >= 1 ) && ( tiles[ i ]->getType() <= TOTAL_TILE_SPRITES ) && tiles[i]->getType() != 39 )
        {
